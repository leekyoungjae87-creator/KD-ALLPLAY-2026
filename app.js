...
if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw.js?v=76.19',{updateViaCache:'none'}).catch(()=>{})}
...
function luckyStatusRender(){const el=document.getElementById('luckyStatus');if(!el)return;const st=luckyPeriod();el.innerHTML=st==='open'?'<div class="open">🟢 지금 응모할 수 있어요 · 9월 30일까지 상품 변경 가능</div>':st==='before'?'<div class="before">⏳ 행운권 응모는 9월 14일부터 시작됩니다.</div>':'<div class="closed">🔒 응모가 마감되었습니다 · 10월 1일 체육한마당에서 당첨자를 공개합니다.</div>'}
function luckyAuthRender(){const el=document.getElementById('luckyAuth'),app=document.getElementById('luckyApp');if(!el||!app)return;if(luckyMe){el.classList.add('hidden');app.classList.remove('hidden');luckyAppRender();return}app.classList.add('hidden');el.classList.remove('hidden');el.innerHTML=`<div class="lucky-login-card"><span class="step">STEP 01 · MY TICKET</span><h3>행운권 응모하기</h3><p>학번·이름과 나만의 4자리 PIN (숫자) (숫자)을 입력해 주세요.</p><div class="lucky-fields"><input id="luckySid" inputmode="numeric" maxlength="5" placeholder="학번 (예: 30715)"><input id="luckyName" maxlength="10" placeholder="이름"><input class="full" id="luckyPin" type="password" inputmode="numeric" maxlength="4" placeholder="4자리 PIN (숫자) (숫자)"></div><button class="lucky-primary" id="luckyEnter">행운권 확인 · 응모하기</button><div class="lucky-subnote">🔐 이미 참여한 학생은 <b>학번 + PIN</b>으로 다시 들어옵니다.<br>PIN을 잊었거나 내가 설정하지 않은 PIN이 등록되어 있다면 본부에서 초기화할 수 있습니다.</div></div>`;document.getElementById('luckyEnter').onclick=luckyEnter}
async function luckyEnter(){const sid=document.getElementById('luckySid').value.trim(),name=document.getElementById('luckyName').value.trim(),pin=document.getElementById('luckyPin').value.trim();if(!/^\d{4,5}$/.test(sid)){alert('학번을 숫자로 정확히 입력해 주세요.');return}if(name.length<2){alert('이름을 입력해 주세요.');return}if(!/^\d{4}$/.test(pin)){alert('PIN은 숫자 4자리로 입력해 주세요.');return}try{let row=await luckyFind(sid),hash=await luckyHash(pin);if(row){if(row.pin_hash!==hash){alert('PIN이 맞지 않습니다. 본인이 설정하지 않은 PIN이라면 본부에 초기화를 요청해 주세요.');return}if(String(row.student_name||'').trim()!==name){alert('처음 등록한 이름과 일치하지 않습니다.');return}luckyMe=row}else{if(luckyPeriod()==='closed'){alert('행운권 응모가 마감되었습니다.');return}await luckyCreate(sid,name,hash);luckyMe=await luckyFind(sid)||{student_id:sid,student_name:name,prize_id:null,pin_hash:hash}}luckyAuthRender()}catch(e){console.error(e);alert('등록 중 문제가 발생했습니다. 잠시 후 다시 시도해 주세요.')}}
