...
const eventRules = [
  {
    cat:"이벤트", name:"학급 응원 퍼포먼스", people:"학급 전체", type:"응원전 · 별도 시상",
    rules:[
      "학급 전체가 참여하는 것을 원칙으로 하며 학급별 2분 이내로 실시",
      "입장 → 퍼포먼스 → 퇴장 순으로 진행",
      "타인 비방, 불쾌감을 주거나 선정적인 내용은 금지",
      "참여도·협동성·창의성·완성도·호응도를 각 20점 기준으로 평가",
      "교직원 투표로 학년별 최우수·우수·장려 1학급씩 시상",
      "종합점수에는 포함하지 않고 별도 시상"
    ]
  },
  {
    cat:"순환", name:"8자 줄넘기", people:"14명", type:"결선 · 기록 경기",
    rules:[
      "남학생 6명·여학생 6명 + 줄잡이 2명",
      "학생 2명이 줄을 돌리고 12명이 8자 모양으로 1명씩 줄넘기",
      "줄 길이는 학급별 자유이며 시작 신호와 함께 줄을 돌리기 시작",
      "2분 동안 실시하고 총 2회 기록을 합산",
      "경기 시작 전 연습 1회 실시",
      "1·2·3·4반 1조 / 5·6·7·8반 2조로 진행",
      "기록이 같으면 공동 순위 처리"
    ]
  },
  {
    cat:"순환", name:"슈팅 릴레이", people:"14명", type:"결선 · 기록 경기",
    rules:[
      "남학생 6명·여학생 6명 + 후보 2명",
      "야외 농구장에서 2학급씩 동시에 진행",
      "지정 라인에서 농구 슛 성공 횟수로 순위를 결정",
      "지정 라인을 밟거나 넘어서 슛하면 무효",
      "남녀 슛 위치는 동일하며 선수 순서는 자유",
      "1회 99초, 총 2회 실시 후 성공 횟수를 합산",
      "시작 전 연습 1분 실시, 기록이 같으면 공동 순위 처리"
    ]
  },
  {
    cat:"순환", name:"바운드 배구", people:"20명", type:"결선 경기",
    rules:[
      "남학생 8명·여학생 8명 + 후보 남녀 각 2명",
      "전반 12분은 여학생, 후반 12분은 남학생 경기",
      "일반 배구 규칙을 기본으로 하되 바운드를 허용",
      "서브는 코트 가운데에서 두 손으로 던져서 실시",
      "회전을 주거나 지나치게 길고 짧게 넣는 공격성 서브 금지",
      "전반과 후반 점수를 합산하여 승부 결정"
    ]
  },
  {
    cat:"단체", name:"2인 3각", people:"16명", type:"학년별 기록 경기",
    rules:[
      "남학생 8명·여학생 8명, 2인 1조 × 8팀",
      "여-남-여-남-여-남-여-남 순서로 진행",
      "2인 1조가 다리를 묶고 출발해 반환점(라바콘)을 돌아옴",
      "돌아온 2인 1조가 다음 조에게 배턴 전달",
      "마지막 주자는 조끼를 착용하여 구분",
      "반환점을 정상적으로 돌지 않거나 배턴 전달 전 출발하면 반칙 처리",
      "마지막 조가 결승선을 통과한 기록으로 순위 결정(0.1초 단위)"
    ]
  },
  {
    cat:"단체", name:"달리는 줄다리기", people:"학급 전원", type:"토너먼트 · 3판 2선승",
    rules:[
      "예선을 통과한 학년별 4팀이 토너먼트로 진행",
      "학급 전원이 참여하되 경기 시 인원이 적은 반과 동일하게 맞추며 남녀 비율도 맞춤",
      "한 경기 시간은 1분, 3판 2선승제",
      "시작 전 여학생은 줄을 잡고 남학생은 상대 팀 줄 끝에 맞춰 라인에 위치",
      "휘슬이 울리면 여학생은 줄을 당기고 남학생은 자신의 줄 표시까지 달려와 합류",
      "남학생이 달려오는 방향은 오른쪽으로 통일"
    ]
  },
  {
    cat:"이벤트", name:"미션 이어달리기", people:"6명", type:"이벤트 경기",
    rules:[
      "남학생 3명·여학생 3명",
      "미션 6개를 차례로 클리어",
      "남-여-남-여-남-여 순서로 진행",
  ...
  {
    cat:"단체", name:"이어달리기", people:"8명", type:"결선 경기",
    rules:[
      "남학생 4명·여학생 4명",
      "여-남-여-남-여-남-여-남 순서로 진행",
      "출발 지점은 본부석 조회대 앞쪽",
      "각 주자 반바퀴씩 총 8명이 달림",
      "마지막 주자가 결승선까지 주행",
      "골인 지점은 공원 쪽 수돗가(2학년 응원석)"
    ]
  },
  {
    cat:"사전", name:"축구(남)", people:"9명", type:"사전 경기",
    rules:[
      "9명 참가(교체선수 1명 포함)",
      "사전경기로 진행하되 종합점수에 포함",
      "예선은 전·후반 구분 없이 15분 경기",
      "결승은 전반 10분 + 후반 10분",
      "무승부일 경우 키커 5명의 승부차기로 결정"
    ]
  },
  {
    cat:"사전", name:"피구(여)", people:"학급별", type:"사전 경기",
    rules:[
      "인원이 적은 학급을 기준으로 참가 인원을 동일하게 맞춤(외야 1명)",
      "사전경기로 진행하되 종합점수에 포함",
      "5분 × 3세트, 한 팀이 2세트를 먼저 이기면 경기 종료",
      "패스는 3회까지 허용",
      "외야↔외야, 내야↔내야 패스 금지",
      "두 발이 경기장 밖으로 나가면 아웃",
      "더블 아웃은 적용하지 않음"
  ...
  {
    cat:"평가", name:"학급 깃발", people:"학급", type:"교직원 평가",
    rules:[
      "전 교직원 투표로 평가",
      "학급의 개성과 단합을 표현한 깃발을 대상으로 평가",
      "종합점수에 반영",
      "배점: 1위 30점 · 2위 20점 · 3위 10점"
    ]
  }
];
